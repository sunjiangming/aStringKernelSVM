...
#ifdef _STRING
	static double edit(const char *px, const char *py); // replace int as double, Jiangming
...
#ifdef _STRING
double Kernel::edit(const char *px, const char *py) // replace int as double ,Jiangming Sun
{
	int* row[2];
	int len1,len2;
	int now,old;
	int i,j;
	int eidt_dist_result = 0;

	len1=(int)strlen(px); len2=(int)strlen(py);

	int center1= (int)round( (float)(len1-1)/2 + 0.5);
	int center2= (int)round( (float)(len2-1)/2 + 0.5);
	
	if(ne_len> center1-1 || ne_len>center2-1)
		ne_len = min(center1-1,center2-1);
	if(nh_len> center1-1 || nh_len>center2-1)
		nh_len = min(center1-1,center2-1);
	
	if(dist_weight>0)
	{
		row[0] = (int*) malloc( (2*ne_len+1+1) * sizeof(int) );
		row[1] = (int*) malloc( (2*ne_len+1+1) * sizeof(int) );

		for(i=center1-ne_len-1;i<center1+ne_len+1;i++) row[0][i-(center1-ne_len-1)]=i-(center1-ne_len-1);
		now=1; old=0;

		for(j=center2-ne_len-1;j<center2+ne_len;j++) /* j=0 is the first char of py */
		{
			row[now][0]=j-(center2-ne_len-1)+1;
			for(i=center1-ne_len-1;i<center1+ne_len;i++) /* i=0 is the first char of px */
			{
				row[now][i-(center1-ne_len-1)+1] = min(min( row[old][i-(center1-ne_len-1)] + (px[i]==py[j]?0:1) , 
						 row[old][i-(center1-ne_len-1)+1] + 1),
					    row[now][i-(center1-ne_len-1)] + 1                        );
			}

		now=1-now; old=1-old;
		}

		eidt_dist_result=row[old][2*ne_len+1];
   
		free(row[0]); free(row[1]);
	}
		
	int hamming_dist_result = 0;
	if(dist_weight<1)
	{
		for(int ii=0;ii<2*nh_len+1;ii++)
		{
			if(px[center1-nh_len-1+ii] != py[center2-nh_len-1+ii])
			++hamming_dist_result ;
		}
	}
	double result = ( dist_weight*eidt_dist_result/(2*ne_len+1) + (1.0-dist_weight)*hamming_dist_result/(2*nh_len+1) );
	//double result = ( dist_weight*eidt_dist_result + (1.0-dist_weight)*hamming_dist_result );
	//double result = hamming_dist_result/eidt_dist_result;
	return result;
}
#endif
